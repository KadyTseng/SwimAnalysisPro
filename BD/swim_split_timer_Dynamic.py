...
class LiveStreamTuningApp:
    ...
    def _create_widgets(self):
        ...
        # 🌟 手動微調面板 (預設隱藏)
        self.manual_panel = ttk.Frame(self.left_panel)
        for color, var in [('Blue', self.blue_var), ('Green', self.green_var), ('Red', self.red_var)]:
            ttk.Label(self.manual_panel, text=f"【手動微調 {color}】", font=('Arial', 9, 'bold')).pack(anchor=tk.W, pady=(5, 0))
            f = ttk.Frame(self.manual_panel)
            f.pack(anchor=tk.W, pady=(2, 5))
            c_let = color[0]
            ttk.Radiobutton(f, text=f"{c_let} - {c_let}ref (正)", variable=var, value=0, command=self._on_manual_change).pack(side=tk.LEFT, padx=5)
            ttk.Radiobutton(f, text=f"{c_let}ref - {c_let} (負)", variable=var, value=1, command=self._on_manual_change).pack(side=tk.LEFT, padx=5)
    ...
    def toggle_lock(self):
        ...
        if not self.is_locked:
            ...
            # 同步鎖定那一刻的數值到 Radiobutton
            self.blue_var.set(self.locked_combo[0])
            self.green_var.set(self.locked_combo[1])
            self.red_var.set(self.locked_combo[2])
            # 顯示手動微調面板
            self.manual_panel.pack(anchor=tk.W, fill=tk.X, pady=10)
    ...
    def _on_manual_change(self):
        if self.is_locked:
            self.locked_combo = (self.blue_var.get(), self.green_var.get(), self.red_var.get())
            formula_str = self._get_formula_text(self.locked_combo)
            self.lbl_composite_formula.config(text=f"🔒 [已鎖定-手動微調]\n{formula_str}", fg='#7f0000', bg='#ffebee')
        ...
        if roi.size > 0:
            ...
            if not self.is_bg_ready:
                ...
                if self.is_locked:
                    ...
                    self.lbl_composite_formula.config(text=f"🔒 [已鎖定公式]\n{formula_str}", fg='#7f0000', bg='#ffebee')
                else:
