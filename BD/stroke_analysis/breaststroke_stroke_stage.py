# SwimAnalysisPro/BD/stroke_analysis/breaststroke_stroke_stage.py
import numpy as np
import cv2
        ...
        for line in f:
            ...
            if len(parts) > 19:
                ...
                if frame_range[0] <= frame_id <= frame_range[1]:
                    ...
                    data.append(
                        (frame_id, col7, col8, col10, col11, col13, col14, col16, col17)
                    )
...
def count_breaststroke_strokes(phase_frames_dict):
    """
    計算蛙式划手次數：統計所有 range 中 'recovery_ends' 的總數。

    參數:
        phase_frames_dict (dict): 包含划水階段幀數的字典。

    回傳:
        dict: 包含總次數、分段次數和詳細階段幀數的字典。
    """
    total_strokes = 0

    # 統計 range1 (去程) 的次數
    recovery_range1 = phase_frames_dict.get("range1", {}).get("recovery_ends", [])
    strokes_range1 = len(recovery_range1)

    # 統計 range2 (回程) 的次數
    recovery_range2 = phase_frames_dict.get("range2", {}).get("recovery_ends", [])
    strokes_range2 = len(recovery_range2)
    ...
    return {
        ...
        "stroke_frames": recovery_range1 + recovery_range2,  # 合併所有划手結束幀
    }
...
#     e1, s2, e2, touch_frame, _ = extract_stroke_segments(
...
#         propulsion_ends1,
#         recovery_ends1,
#     ) = process_range(txt_path, range1, "neg2pos")
#     (
#         frames2,
#         wrist_x2,
#         wrist_y2,
...
#         propulsion_ends2,
#         recovery_ends2,
#     ) = process_range(txt_path, range2, "pos2neg")
